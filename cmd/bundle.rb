    ...
    class BundleCmd < AbstractCommand
      cmd_args do
        usage_banner <<~EOS
          ...
          Remove entries that match `name` from your `Brewfile`. Use `--formula`, `--cask`, `--tap`, `--mas`, `--whalebrew` or `--vscode` to remove only entries of the corresponding type. Passing `--formula` also removes matches against aliases and old formula names.
